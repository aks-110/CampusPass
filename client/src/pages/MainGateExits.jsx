...
import { LogOut, Search, FileText, Download } from 'lucide-react';
...
const MainGateExits = () => {
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "ExitsLog");
    XLSX.writeFile(workbook, "Today_Gate_Exits_List.xlsx");
  };

  const fetchExits = async () => {
    setLoading(true);
    try {
      const { data } = await axiosInstance.get('/gate/logs?action=Exit');
      setLogs(data);
    } catch (error) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold text-primary">Today's Exits</h2>
          <p className="text-muted-foreground">List of students who checked out of campus through the gates today.</p>
        </div>
        <button 
      ...
      {loading ? (
        <div className="p-12 text-center text-muted-foreground animate-pulse border border-border rounded-xl bg-card">Loading exits...</div>
      ) : (
        <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
          <div className="p-4 border-b border-border bg-secondary/10 flex justify-between items-center">
            <h3 className="font-semibold text-lg">Outward Scans Today</h3>
            <span className="bg-primary text-primary-foreground text-xs font-bold px-2.5 py-1 rounded-full flex items-center gap-1">
              <LogOut size={12} /> {filteredLogs.length} Checked Out
            </span>
          </div>
          ...
          {filteredLogs.length === 0 ? (
            <div className="p-12 text-center text-muted-foreground flex flex-col items-center gap-2">
              ...
              <p>No exit records found for today.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm text-left">
                <thead className="bg-secondary/5 text-muted-foreground uppercase text-xs">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-right">Exit Log Time</th>
                  </tr>
                </thead>
